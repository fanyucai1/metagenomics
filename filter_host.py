...
cmd+="/software/bowtie2-2.4.5-linux-x86_64/bowtie2 --very-sensitive-local --no-sq -p 48 -x /reference/%s -1 //raw_data/%s -2 /raw_data/%s " \
    "--un-conc-gz /outdir/%s.%s.unmapped.reads -S /outdir/%s.%s.mapped.sam" \
     %(host_index,a,b,args.prefix,host_index,args.prefix,host_index)
print(cmd)
